...
type tournamentMergeIterator struct {
	iters   []sstable.Iterator
	states  []tournamentMergeState
	tree    []int
	size    int
	current *mergeEntry
	lastKey []byte
...
type tournamentMergeState struct {
	key     []byte
	value   []byte
	trailer uint64
	seq     uint64
...
func newTournamentMergeIterator(iters []sstable.Iterator) *tournamentMergeIterator {
	mi := &tournamentMergeIterator{
		iters:  iters,
		states: make([]tournamentMergeState, len(iters)),
	}
...
func (mi *tournamentMergeIterator) loadState(i int, ikey *sstable.InternalKey, val []byte) {
	state := &mi.states[i]
...
func (mi *tournamentMergeIterator) winner(a, b int) int {
	if a == -1 {
		return b
	}
	if b == -1 {
...
func (mi *tournamentMergeIterator) advance(i int) {
	kv := mi.iters[i].Next()
	if kv != nil {
		mi.loadState(i, &kv.K, kv.InPlaceValue())
		mi.tree[mi.size+i] = i
...
func (mi *tournamentMergeIterator) Next() bool {
...
func (mi *tournamentMergeIterator) key() []byte {
	if mi.current == nil {
		return nil
	}
...
func (mi *tournamentMergeIterator) seq() uint64 {
	if mi.current == nil {
		return 0
	}
...
func (mi *tournamentMergeIterator) kind() pebble.InternalKeyKind {
	if mi.current == nil {
...
func (mi *tournamentMergeIterator) value() []byte {
	if mi.current == nil {
		return nil
	}
...
func (mi *tournamentMergeIterator) entry() (internal.CompactionEntry, error) {
	if mi.current == nil {
...
func (mi *tournamentMergeIterator) Err() error {
	if mi.err != nil {
		return mi.err
	}
...
func (mi *tournamentMergeIterator) close() error {
	var firstErr error
	for _, iter := range mi.iters {
